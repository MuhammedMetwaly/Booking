...
$('.steps input[type=radio]').change(function () {

  if ($(this).attr('id') === 'wage_yes') {
    $('div.wage_yes').removeClass('d-none')
    $('div.wage_yes').find('input, select').attr({
      'data-rule-required': true,
      'required': true,
    })

  } else if ($(this).attr('id') === 'wage_no') {
    $('div.wage_yes').addClass('d-none')
    $('div.wage_yes').find('input, select').attr({
      'data-rule-required': false,
      'required': false,
    })

  } else if ($(this).attr('id') === 'p_no') {
    $('div.p_no').removeClass('d-none')
    $('div.p_no').find('input, select').attr({
      'data-rule-required': true,
      'required': true,
    })
  } else if ($(this).attr('id') === 'p_yes') {
    $('div.p_no').addClass('d-none')
    $('div.p_no').find('input, select').attr({
      'data-rule-required': false,
      'required': false,
    })
  } else if ($(this).attr('id') === 'host_work') {
    $('div.host_work').removeClass('d-none')
    $('div.host_work').find('input, select').attr({
      'data-rule-required': true,
      'required': true,
    })

  } else if ($(this).attr('id') === 'host_nowork') {
    $('div.host_work').addClass('d-none')
    $('div.host_work').find('input, select').attr({
      'data-rule-required': false,
      'required': false,
    })

  } else if ($(this).attr('id') === 'child_work') {
    $('div.child_work').removeClass('d-none')
    $('div.child_work').find('input, select').attr({
      'data-rule-required': true,
      'required': true,
    })

  } else if ($(this).attr('id') === 'child_nowork') {
    $('div.child_ work').addClass('d-none')
    $('div.child_work').find('input, select').attr({
      'data-rule-required': false,
      'required': false,
    })

  } else if ($(this).attr('id') === 'child_home') {
    $('div.child_home').removeClass('d-none')
    $('div.child_home').find('input, select').attr({
      'data-rule-required': true,
      'required': true,
    })

  } else if ($(this).attr('id') === 'child_nohome') {
    $('div.child_home').addClass('d-none')
    $('div.child_home').find('input, select').attr({
      'data-rule-required': false,
      'required': false,
    })

  } else if ($(this).attr('id') === 'car_other') {
    $('div.car_other').removeClass('d-none')
    $('div.car_other').find('input, select').attr({
      'data-rule-required': true,
      'required': true,
    })

  } else if ($(this).attr('id') === 'car_yes' || 'car_no') {
    $('div.car_other').addClass('d-none')
    $('div.car_other').find('input, select').attr({
      'data-rule-required': false,
      'required': false,
    })
...
$('.steps .dropdown-item').click(function (e) {
  e.stopPropagation()
})

let myArray = []
$('.salary_checks input').change(function () {
  $(".salary_checks input:checked").each(function () {
    myArray.push($(this).val());
  });
  $('.collectSalary').val(myArray.join(', '))
})
modules.init();